<template>
    <div class="flex flex-col justify-between gap-4 w-full p-4 bg-neutral-100 dark:bg-neutral-900 rounded-md">
        <div class="flex flex-row items-center justify-between gap-4">
            <p class="text-center">{{ $t('settings.language') }}</p>
        ...
        <div class="flex my-1 flex-row items-center justify-between gap-4">
            <p class="text-center">{{ $t('settings.sounds') }}</p>
            <USwitch size="lg" v-model="soundOn" @change="handleSoundOnChange" />
        </div>
        <div class="flex flex-row items-center justify-between gap-4">
            <p :data-sound="soundOn"
                class="text-center data-[sound=false]:text-neutral-500 data-[sound=false]:cursor-not-allowed">
            {{ $t('settings.beep') }}</p>
            <USelect size="lg" v-model="selectedNotif" :items="notificationSounds" :disabled="!soundOn"
                @change="handleNotifChange" class="hover:cursor-pointer" />
        </div>
    </div>
</template>
